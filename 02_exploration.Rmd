...
This project investigates the increase in game duration and the effect of recent efforts to quicken the pace. Games today last nearly twice as long as they did in 1901. Baseball-Reference has a rich source of game features that might help explain trends in game time. The Baseball-Reference section for the [Cleveland Guardians](https://www.baseball-reference.com/teams/CLE/2023.shtml) lists game-by-game results since their first season in 1901. In [step 01](https://github.com/mpfoley73/cleveland-mlb-games/blob/main/01_get_games.R) of this project I aggregated `r n_distinct(cle_games$season)` seasons of data sets, including YTD for 2023, into data frame `cle_games`. `cle_games` is a `r scales::comma(nrow(cle_games))` x `r ncol(cle_games)` data frame with one row per game. I also downloaded player season summaries of hitting and pitching statistics. `cle_batting` is a `r scales::comma(nrow(cle_batting))` x `r ncol(cle_batting)` data frame and `cle_pitching` is a `r scales::comma(nrow(cle_pitching))` x `r ncol(cle_pitching)` data frame each with one row per season and player. This step explores the data sets.

## The Data Sets

For `cle_games`, columns `game_date` and `game_duration` are obviously going to be important. A few others may be useful too: `day_ind` (day or night game), which game of a `doubleheader_game` if applicable, `runs_scored` and `runs_allowed`, `innings`, and `attendance`. The seven character fields are interesting to read for nostalgia's sake (e.g., Lenny Barker's [1981](https://www.baseball-reference.com/teams/CLE/1981-schedule-scores.shtml) no-hitter on May 15 was against the Blue Jays and the win brought our record to 16-8), but these features won't play a role in this analysis. The lone date field, `game_date`, might be useful in the trend analysis, but the emphasis will be on seasonal roll-ups. I wonder about `day_ind`. Compared to lazy afternoons, evening games feel electric and I wonder if that translates into shorter games. I also wonder about `doubleheader_game`. When I played, I felt like two hours of baking in the sun was plenty, but did I do anything to hasten along a doubleheader? 

```{r}
skimr::skim(cle_games)
```

For `cle_batting`, I will be interested in how the 2023 rule change that increased the base size affected stolen bases, `SB` and `CS`. And did the restrictions on the shift increase batting average, `BA`, and reduce the incentive to hit home runs, `BA`?

```{r}
skimr::skim(cle_batting)
```

I don't anticipate using `cle_pitching` since everything related to game duration and the effects of rules on pace of game are already captured in the batting statistics. Here is the data set anyway.
...
I will use the **tsibble** package for time series analysis. To get an accurate measure of pace of play, I will standardize variables to per-nine-innings.

```{r echo=TRUE}
# Batting and pitching are at the player/season granularity. Summarize to 
# team/season.
cle_batting_season <- cle_batting %>%
  mutate(`1B` = H - (`2B` + `3B` + HR)) %>%
  rename(yr = season) %>%
  summarize(
    .by = yr,
    Age = weighted.mean(Age, G),
    across(c(PA:SO, TB:`1B`), ~sum(.x, na.rm = TRUE))
  ) %>%
  mutate(
    BA = H / AB,
  ...
  rename_with(~ paste0("batting_", .x), c(Age:OPS))

cle_pitching_season <- cle_pitching %>%
  rename(yr = season) %>%
  summarize(
    .by = yr,
    Age = weighted.mean(Age, G),
    across(c(CG:BF), ~sum(.x, na.rm = TRUE))
  ) %>%
  mutate(
    ERA = 9 * ER / IP,
    WHIP = (BB + H) / IP,
  ...
  select(-c(IP)) %>%
  rename_with(~ paste0("pitching_", .x), c(Age:SO_per_BB))

# Standardize games to a 9-inning game
cle_games_per9 <- cle_games %>%
  mutate(
    game_hrs = as.duration(game_duration) / dhours(1),
...
cle_batting_tsibble_yr <- tsibble(cle_batting_season, index = yr)
cle_pitching_tsibble_yr <- tsibble(cle_pitching_season, index = yr)

# key includes doubleheader game in order to index by game.
cle_tsibble <- tsibble(cle_games_per9, key = c(doubleheader_game), index = game_date)
