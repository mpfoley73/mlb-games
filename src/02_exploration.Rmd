...
The MLB API appears to have the most complete record of game duration (Fig `r fig_no + 1`a **Missingness**), but keep in mind that this data is pre-summarized at the season level, so 100% coverage since 1900 just means the API returned a sum of minutes for each season. (Fig `r fig_no + 1`b **Trend**) shows that when you divide by innings, something is quite off for eight of the seasons. I am disinclined to throw out the bathwater lest a perfectly good baby is floating in there, so I imputed values using MICE. A more sensible procedure would be to use values from the other sources, but I wanted MICE practice. More on how to use MICE in [my blog](https://mpfoley73.netlify.app/post/2022-07-26-imputing-values-with-mice/). Fig `r fig_no + 1`d **MLB with Imputations**) shows the imputations.
...
Otherwise, fig `r fig_no + 1`e **New Trend** looks reasonable, but there is still unexplained disagreement for 1918. MLB API pops up while Retrosheet Event drops. Baseball-Reference and Retrosheet Game are fairly close. I will keep this in mind, but not tinker with it (I don't think it will be wise to rely too closely on data that far back.
...
<font size=-1 color="#666666"><b>Fig. `r (fig_no <- fig_no + 1)`a. Trend</b>. MLB API plate appearances per nine-innings diverges from other sources prior to 1910, but all values seem strange prior to 1920.</font><br>
...
<font size=-1 color="#666666"><b>Fig. `r fig_no`b. Trend</b>. MLB API data imputed for 1901-1911.</font><br>

## Pitches {.tabset .tabset-fade .tabset-pills}

There is no recorded pitch counts until 1950 and until 1988 the values are less than half of what seems reasonable (Fig `r fig_no + 1`a **Trend**). The 1988-1998 values are still low and variable. Only 1999-present are consistent. I am nulling out all pitch counts prior to 1999. The pared down data set shows a small upward trend of about 10 additional pitches per nine innings since 1999 (Fig `r fig_no + 1`b **New Trend**). The MLB-API data is lower by about 8 pitches per nine-innings. I suspect the API does not account for games when the home team does not bat in the bottom of the inning. I will leave the data alone, but note the difference.
...
<font size=-1 color="#666666"><b>Fig. `r fig_no <- fig_no + 1`a. Trend</b>. Pitch count per nine-innings. Shapes indicate plausibility.</font><br>
...
The three sources of pitcher counts are consistent. The MLB API and Retrosheet game (g) start in 1901, and Retrosheet event (e) starts in 1914 (Fig `r fig_no + 1`a **Total**). The Retrosheet event logs provide details about when pitching substitutions take place (Fig `r fig_no + 1`b **Mid-Inning Substitutions**). The drop-off of begin-inning substitutions in 1973 coincides with the introduction of the designated hitter, so that makes sense. The temporary increase in mid-inning substitutions in 2020 might have something to do with COVID. 
...
```{r warning=FALSE}
retro_season_e_ts %>%
  rename(Total = pitchers_9I, `Mid-inning` = pitsubsmi_9I, `Start-inning` = pitsubsbi_9I) %>%
...
The stolen base data is less important because it shouldn't affect game duration, but changes to its frequency are a bi-product of the 2023 rule changes related to base width and pick-off attempts. The caught-steeling data prior to 1926 is clearly not to be trusted, but what about the caught-stealing data from 1926 - 1950 (Fig `r fig_no + 1`a. Stolen bases per nine innings? I'm inclined to null out all of the caught-steeling data prior to 1951. There are also two outliers in 1914 and 1915. I don't have an explanation for why they would pop up like that and it's a big change from the prior trend. The 1914 outlier looks like it could be stolen base _attempts_. I will impute values for 1914 and 1915 (Fig `r fig_no + 1`b).
...
# For plots
lbl_yr <- c(1901, 2022)
retro_1901 <- retro_season_g_ts %>% filter(season == 1901)

plot_trend_vs_dur <- function(dat) {
  dat %>%
    mutate(std_hrs_9I = hrs_9I / retro_1901$hrs_9I) %>%
    ggplot(aes(x = season)) +
...
Plate appearances (PA) per nine innings of play have changed little over time (Fig. `r (fig_no + 1)`). Walks increased with game duration, at least through the 1940s, suggesting strategy was becoming increasingly important (Fig. `r (fig_no + 2)`). Strikeouts in particular have increased with game times (Fig. `r (fig_no + 3)`). Of the main offense metrics, strikeouts come closest to matching the trend in game duration. Strikeouts alone can explain about `r percent(r2_K, 1)` of the variation in average game duration (Fig. `r (fig_no + 4)`). Finally, hits (H) have fluctuated around 17 per nine innings since the 1940s (Fig. `r (fig_no + 1)`).

### PA

```{r warning=FALSE}
retro_season_g_ts %>%
  mutate(act_y = PA_9I, std_y = PA_9I / retro_1901$PA_9I) %>%
  plot_trend_vs_dur() +
  ggtitle("Changes in plate appearances and game duration, 1901-2022.")
```

<font size=-1 color="#666666"><b>Fig. `r (fig_no <- fig_no + 1)`. Plate appearances (orange) and game duration (gray), per nine-innings</b>. Plate appearances are nearly unchanged.<br> Source: Retrosheet Game.</font>

### BB

```{r warning=FALSE}
retro_season_g_ts %>%
  mutate(act_y = BB_9I, std_y = BB_9I / retro_1901$BB_9I) %>%
  plot_trend_vs_dur() +
  ggtitle("Changes in walks and game duration, 1901-2022.")
```

<font size=-1 color="#666666"><b>Fig. `r (fig_no <- fig_no + 1)`. Walks (orange) and game duration (gray), per nine-innings</b>. Walks increased with game duration until 1950.<br> Source: Retrosheet Game.</font>

### SO

```{r warning=FALSE}
retro_season_g_ts %>%
  mutate(act_y = SO_9I, std_y = SO_9I / retro_1901$SO_9I) %>%
  plot_trend_vs_dur() +
  ggtitle("Changes in strikeouts and game duration, 1901-2022.")
```

<font size=-1 color="#666666"><b>Fig. `r (fig_no <- fig_no + 1)`. Strikeouts (orange) and game duration (gray), per nine-innings</b>. Strikeouts climbed gradually after the dead-ball era, then more sharply after WWII. They declined from the mid-60's to 1980, but have climbed since then, peaking at `r comma(max_K[1, ]$SO_9I, .1)` in `r max_K[1, ]$season`.<br> Source: Retrosheet Game.</font>
...
```{r warning=FALSE}
retro_season_g_ts %>%
  mutate(act_y = H_9I, std_y = H_9I / retro_1901$H_9I) %>%
  plot_trend_vs_dur() +
  ggtitle("Changes in hits and game duration, 1901-2022.")
```

<font size=-1 color="#666666"><b>Fig. `r (fig_no <- fig_no + 1)`. Hits (orange) and game duration (gray), per nine-innings</b>. Hits decreased to about 17 by 1942 and have varied little since then.<br> Source: Retrosheet Game.</font>
