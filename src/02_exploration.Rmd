...
<font color = "#666666" size = -1><b>Table `r (tbl_no <- tbl_no + 1)`</b>. Coverage for data sources used in this analysis.</font>
...
retro_season_e_ts <- retro_events %>%
  ...
  summarise(
    ...
    G = n(),
    I_G = sum(inn_ct) / G,
    PA_9I = mean(PA / inn_ct * 9, na.rm = TRUE),
    PA_pitcher = mean(PA / pitchers, na.rm = TRUE),
    pitchers_9I = mean(pitchers / inn_ct * 9, na.rm = TRUE),
    pitsubsmi_9I = mean(mid_inning_pitcher_sub_ct / inn_ct * 9, na.rm = TRUE),
    P_9I = mean(P / inn_ct * 9, na.rm = TRUE),
    ...
    R_9I = mean((away_score_ct + home_score_ct) / inn_ct * 9, na.rm = TRUE),
    R_PA = mean((away_score_ct + home_score_ct) / PA, na.rm = TRUE),
    H_9I = mean((away_hits_ct + home_hits_ct) / inn_ct * 9, na.rm = TRUE),
    H_PA = mean((away_hits_ct + home_hits_ct) / PA, na.rm = TRUE),
    HR_9I = mean((away_hr_ct + home_hr_ct) / inn_ct * 9, na.rm = TRUE),
    HR_PA = mean((away_hr_ct + home_hr_ct) / PA, na.rm = TRUE),
    BB_9I = mean((away_bb_ct + home_bb_ct) / inn_ct * 9, na.rm = TRUE),
    BB_PA = mean((away_bb_ct + home_bb_ct) / PA, na.rm = TRUE),
    SO_9I = mean((away_so_ct + home_so_ct) / inn_ct * 9, na.rm = TRUE),
    SO_PA = mean((away_so_ct + home_so_ct) / PA, na.rm = TRUE),
    SB_9I = mean((away_sb_ct + home_sb_ct) / inn_ct * 9, na.rm = TRUE),
    CS_9I = mean((away_cs_ct + home_cs_ct) / inn_ct * 9, na.rm = TRUE),
    ERA = mean((away_er_ct + home_er_ct) / inn_ct * 9, na.rm = TRUE),
    WHIP = mean((away_bb_ct + home_bb_ct + away_hits_ct + home_hits_ct) / inn_ct, na.rm = TRUE),
...
```{r}
retro_season_g_ts %>% 
  replace_na(list(att_G = 0)) %>%
  ggplot(aes(x = season, y = att_G)) +
  geom_line(data = bref_cle_season_ts %>% replace_na(list(home_att_G = 0)),
            aes(y = home_att_G),
            color = pal$cle["R"], linewidth = 2.5, alpha = .20) +
  geom_line(color = pal$sfg["O"], linewidth = 1) +
  geom_point(data = retro_season_g_ts %>% filter(season %in% c(1995, 2007)),
             shape = 21, color = pal$sfg["O"], fill = "white", size = 3, na.rm = TRUE) +
  geom_text(data = retro_season_g_ts %>% filter(season %in% c(1995, 2007)), 
  ...
  labs(
    x = NULL, y = "Fans per Game",
    title = "MLB Attendance still recovering from COVID.",
    subtitle = "Average attendance across MLB. Cleveland shown in backdrop."
  )
```

<font size=-1 color="#666666"><b>Fig. `r (fig_no <- fig_no + 1)`</b>. Major League Baseball attendance peaked in 2007. 2022 attendance averaged `r retro_season_g_ts %>% filter(season == 2022) %>% pull(att_G) %>% comma(1)`.<br>Source: MLB API.</font>
