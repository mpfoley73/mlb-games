...
retro_games <- readRDS("../data/retro_game_logs.rds")
```
...
```{r}
retro_season_all <- retro_games %>%
  summarise(
    .by = year_id,
    G = n(),
    avg_mins_G = mean(minutes_game_ct, na.rm = TRUE),
    avg_mins_I = mean(minutes_game_ct / inn_ct, na.rm = TRUE),
    non_na_mins_G = sum(!is.na(minutes_game_ct)),
    avg_att = mean(attend_park_ct, na.rm = TRUE),
    non_na_att_G = sum(!is.na(attend_park_ct)),
...
retro_season_cle <- retro_games %>%
  filter(home_team_id == "CLE" | away_team_id == "CLE") %>%
  summarize(
    .by = year_id,
    G = n(),
    avg_mins_G = mean(minutes_game_ct, na.rm = TRUE),
    avg_mins_I = mean(minutes_game_ct / inn_ct, na.rm = TRUE),
    non_na_mins_G = sum(!is.na(minutes_game_ct)),
    avg_home_att = mean(if_else(home_team_id == "CLE", attend_park_ct, NA_real_), na.rm = TRUE),
    non_na_att_G = sum(home_team_id == "CLE" & !is.na(attend_park_ct)),
    pitchers = sum(away_pitcher_ct + home_pitcher_ct),
    mi_p_subs = sum(mid_inning_pitcher_sub_ct),
    I = sum(inn_ct),
    R = sum(away_score_ct + home_score_ct),
...
retro_game <- tbl(retrosheet_conn, "game")
cle_att <- retro_game %>% 
  filter(home_team_id == "CLE") %>%
  select(year_id, attend_park_ct) %>%
  collect()
cle_att %>% filter(is.na(attend_park_ct))
cle_att %>%
  filter(!is.na(attend_park_ct)) %>%
  summarize(.by = year_id, att = mean(attend_park_ct, na.rm = TRUE)) %>%
  ggplot(aes(x = year_id, y = att)) + geom_line()
```
