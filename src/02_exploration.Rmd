...
<font size=-1 color="#666666"><b>Fig. `r (fig_no <- fig_no + 1)`a. Missingness</b>. Only the MLB API had duration data for all seasons back to 1900.</font><br>
...
<font size=-1 color="#666666"><b>Fig. `r fig_no`b. Trend</b>. The MLB API duration data has several problematic data points prior to 1930.</font><br>
...
<font size=-1 color="#666666"><b>Fig. `r fig_no`c. MLB API's Dubious Durations</b>. A closer inspection identifies eight outliers.</font><br>
...
<font size=-1 color="#666666"><b>Fig. `r fig_no`d. MLB with Imputations</b>. Using MICE to impute values produces realistic values.</font>
...
The Baseball-Reference data under-counts plate appearances (PAs) for Cleveland games because my definition of PAs (AB + sacs + BBs) is not quite the same as the [MLB definition](https://www.mlb.com/glossary/standard-stats/plate-appearance) of runs + outs + LOB since I didn't have the LOB. The MLB APA differs from Retrosheet slightly, esp. prior to 1910. Retrosheet looks like the more reliable source here. This time I will impute dubious MLB API values with the realistic ones from Retrosheet (g).

The trough between 1901 and 1920 is strange. Why did PA per 9 fall and rise like that? Unless I can explain this, it seems safer to only trust the data from 1920 forward.

PA has a long gradual decline from 1935-1968 totaling nearly six less batters. At, say, 2:30 per PA, that might reduce game times by 15 minutes, then increase them by 15 minutes when they recovered. They never quite recovered though, and then they declined again after 2000. I don't think changes in PA explain game duration.
...
bind_rows(
  ...
  labs(
    ...
    caption = "Note: Baseball-Reference is Cleveland games only, and uses rough approximation of PA and innings.",
    x = NULL, y = NULL, color = NULL, linewidth = NULL, alpha = NULL
  ) +
  guides(fill = guide_legend(reverse = TRUE)) +
  theme(axis.text.x = element_text(angle = 90, vjust = .5))
...
There is no recorded pitch counts until 1950 and until 1988 the values are less than half of what seems reasonable. The 1988-1998 values are still low and variable. Only 1999-present are consistent. I am nulling out all pitch counts prior to 1999. The pared down data set shows a small upward trend of about 10 additional pitches per nine innings since 1999.
...
The three sources of pitcher counts are consistent. The MLB API and Retrosheet game (g) start in 1901, and Retrosheet event (e) starts in 1914. The Retrosheet event logs provide details about when pitching substitutions take place. The drop-off of begin-inning substitutions in 1973 coincides with the introduction of the designated hitter, so that makes sense. The temporary increase in mid-inning substitutions in 2020 might have something to do with COVID. 
...
<font size=-1 color="#666666"><b>Fig. `r (fig_no <- fig_no + 1)`a. Total</b>. All three sources have pitchers per game. MLB API stretches all the way back to 1900 and includes the current season YTD. Retrosheet games start in 1901. Retrosheet event log data starts in 1914.</font>

### Mid-inning Substitutions

```{r warning=FALSE}
bind_rows(
  `Total` = retro_season_e_ts %>% select(season, y = pitchers_9I) %>% as_tibble(),
  `Mid-inning` = retro_season_e_ts %>% select(season, y = pitsubsmi_9I) %>% as_tibble(),
  `Start-inning` = retro_season_e_ts %>% select(season, y = pitsubsbi_9I) %>% as_tibble(),
  .id = "Source"
) %>%
  ...
  coord_cartesian(ylim = c(0, 10)) +
  scale_color_manual(values = pal$mlb[3:5]) +
  labs(
    title = "Begin-inning pitching substitutions have driven the increased appearances.",
    subtitle = "Avg pitchers per nine innings.",
    x = NULL, y = NULL, color = NULL
  )
...
retro_season_e_ts %>%
  # as_tibble() %>%
  pivot_longer(cols = c(pitchers_9I, pitsubsmi_9I, pitsubsbi_9I), names_to = "Source", values_to = "y") %>%
  ggplot(aes(x = season, y = y, color = Source)) + 
  geom_point() +
  scale_y_continuous(labels = comma_format(1), breaks = 0:10) +
  coord_cartesian(ylim = c(0, 10)) +
...
<font size=-1 color="#666666"><b>Fig. `r fig_no`b. Mid-inning Substitutions</b>. </font>

## Stolen Bases {.tabset .tabset-fade .tabset-pills}

The stolen base data is less important because it shouldn't affect game duration, but changes to its frequency are a bi-product of the 2023 rule changes related to base width and pick-off attempts. The caught-steeling data prior to 1926 is clearly not to be trusted, but what about the caught-stealing data from 1926 - 1950? I'm inclined to null out all of the caught-steeling data prior to 1951. There are also two outliers in 1914 and 1915. I don't have an explanation for why they would pop up like that and it's a big change from the prior trend. The 1914 outlier looks like it could be stolen base _attempts_. I will impute values for 1914 and 1915.
...
<font size=-1 color="#666666"><b>Fig. `r (fig_no <- fig_no + 1)`a. Stolen bases per nine innings</b>.</font>

### Redacted and Imputed
...
Games typically lasted around two hours up to 1945. Then over the course of ten seasons game times shot up to 2.5 hours. In the late 1970s games began another decade-long increase, hitting `r mlb_season_ts %>% filter(season == 1988) %>% pull(hrs_9I) %>% pretty_hm(verbose = TRUE)` in 1988. After 2011, game time began a third period of increase, reaching `r max_avg %>% pull(hrs_9I) %>% pretty_hm(verbose = TRUE)` in `r max_avg %>% pull(season)`. Several rule changes introduced in 2023 appear to have halted the advance. Average game times in 2023 through `r max(bref_cle_games$game_date) %>% format("%b %d, %Y")` have been `r avg_2023 %>% pull(hrs_9I) %>% pretty_hm(verbose = TRUE)`, a `r comma(max_m_23)` minute reduction. 
...
Let's start with plate appearances per nine innings of play (PA_9I). It has not changed much over time (Fig. `r (fig_no + 1)`). However, the offensive outcomes have (Fig. `r (fig_no + 2)`). Strikeouts in particular have increased with game times and alone can explain about `r percent(r2_K, 1)` of the variation in average game duration (Fig. `r (fig_no + 2)`). 
...
```{r warning=FALSE}
lbl_yr <- c(1901, 2023)
hrs_9I_1901 <- mlb_1901 <- mlb_season_ts %>% filter(season == 1901) %>% pull(hrs_9I)
PA_9I_1901 <- mlb_1901 <- mlb_season_ts %>% filter(season == 1901) %>% pull(PA_9I)

mlb_season_ts %>%
  mutate(
    std_hrs_9I = hrs_9I / hrs_9I_1901,
    std_PA_9I = PA_9I / PA_9I_1901,
  ) %>%
  ggplot(aes(x = season)) +
  geom_line(aes(y = std_hrs_9I), color = pal$sfg["B"], linewidth = 2, alpha = 0.15) +
  geom_line(aes(y = std_PA_9I), color = pal$sfg["O"], linewidth = 1) +
  geom_point(aes(y = if_else(season %in% lbl_yr, std_PA_9I, NA_real_)),
             color = pal$sfg["O"], size = 2) +
  geom_text(aes(x = season + 1,
                y = if_else(season %in% lbl_yr, std_PA_9I + .05, NA_real_),
                label = comma(PA_9I, .1)),
  ...
  labs(
    title = "Changes in plate appearances are unrelated to pace of play",
    subtitle = "Changes in plate appearances and game duration since 1901.",
    x = NULL, y = NULL
  )
```
...
mlb_season_ts %>%
  ...
  scale_color_discrete(labels = c(BB_9I = "Walks", H_9I = "Hits", SO_9I = "K's", R_per_9 = "Runs")) +
  labs(
...
## Mid-inning pitching substitutions

From 1914-1972, pitchers per game increased from 3 to 5, driven by mid-inning pitching changes. Then pitcher counts increased more quickly as managers began changing pitchers at the start of innings. Something is amiss in 2020-2022. In 2020, MLB introduced the [three-batter minimum](https://www.rookieroad.com/baseball/three-batter-minimum-rule-0641152/#:~:text=of%20Play%20Rules-,Definition,are%20removed%20from%20the%20game.) rule for relievers. That reduced mid-inning pitching changes by about one. Why did mid-inning changes temporarily revert to trend in 2021? I suspect the explanation
...
retro_season_e_ts %>%
  mutate(
    ...
    std_pitchers_9I = pitsubsmi_9I / mean(pitsubsmi_9I),
...
# Appendix B: Other Clevaland Trends
