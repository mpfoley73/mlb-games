...
(step_2_fig1 <- cle_season_ts %>%
  ...
  geom_col(aes(y = 1 * SF), fill = cle_colors["navy_blue"], width = 1, alpha = 0.2) +
  geom_col(aes(y = wins / (wins + losses) * SF), fill = cle_colors["navy_blue"], width = 1, alpha = 0.2) +
  ...
  geom_hline(yintercept = 0.50 * SF, linetype = 3, linewidth = 1, color = cle_colors["navy_blue"]) +
  annotate(
  ...
  theme(
    ...
    axis.title.y.left = element_text(color = cle_colors["red"], margin = margin(r = 5)),
    axis.title.y.right = element_text(color = cle_colors["navy_blue"], margin = margin(l = 5))
  )
)
```
...
avg_2023 <- cle_season_ts %>% filter(yr == 2023)
```

Games typically lasted around two hours up to 1945. Then over the course of ten seasons game times increased to 2.5 hours. In the late 1970s games began increasing again, peaking at `r max_avg %>% pull(avg_hours_per_9) %>% pretty_hm(verbose = TRUE)` in `r max_avg %>% pull(yr)`. They dropped briefly before resuming their climb through 2021. Significant changes in 2023 appear to have halted the advance. Average Guardians game times in 2023 have been `r avg_2023 %>% pull(avg_hours_per_9) %>% pretty_hm(verbose = TRUE)` through `r avg_2023 %>% pull(games)` games.
...
```{r}
knot_yrs <- c(1913, 1944, 1955, 1978, 2000, 2003, 2021)

model(mlb_season_ts, TSLM(hours_per_9 ~ trend(knots = knot_yrs))) %>%
  augment() %>%
  ggplot(aes(x = season)) +
  geom_line(aes(y = hours_per_9), color = cle_colors["red"], linewidth = 1) +
  geom_line(aes(y = .fitted), color = "black", linetype = 2, linewidth = .5) +
  geom_text(data = mlb_season_ts %>% filter(season %in% c(1945, 2000, 2021, 2023)), 
            aes(y = hours_per_9 + if_else(season == 2023, -.15, +.15), label = pretty_hm(hours_per_9)), 
            size = 3, color = cle_colors["red"], hjust = 0) + 
  geom_point(data = mlb_season_ts %>% filter(season %in% c(1945, 2000, 2021, 2023)), 
             aes(y = hours_per_9),
  ...
  labs(
    ...
    subtitle = "MLB game duration trend.",
    caption = "source: www.baseball-reference.com.",
    x = NULL, y = "Hours"
  )
```
...
fmla_pls <- formula(avg_hours_per_9 ~ trend(knots = knot_yrs))

model(cle_season_ts, TSLM(avg_hours_per_9 ~ trend(knots = knot_yrs))) %>%
  augment() %>%
  ggplot(aes(x = yr)) +
  geom_line(aes(y = avg_hours_per_9), color = cle_colors["red"], linewidth = 1) +
  geom_line(aes(y = .fitted), color = "black", linetype = 2, linewidth = .5) +
  geom_text(data = cle_season_ts %>% filter(yr %in% c(1901, 2000, 2023)), 
            aes(y = avg_hours_per_9 + if_else(yr == 2023, -.15, +.15), label = pretty_hm(avg_hours_per_9)), 
            size = 3, color = cle_colors["red"], hjust = 0) + 
  geom_point(data = cle_season_ts %>% filter(yr %in% c(1901, 2000, 2023)), 
             aes(y = avg_hours_per_9),
             color = cle_colors["red"], size = 2) +
  scale_x_continuous(breaks = seq(1900, 2030, 10)) +
  scale_y_continuous(limits = c(0, NA), n.breaks = 4) +
  coord_cartesian(xlim = c(1900, 2030), ylim = c(0, 3.5)) +
  labs(
    title = "How long have we been sitting here?",
    subtitle = "Cleveland game duration trend.",
    caption = "source: www.baseball-reference.com.",
    x = NULL, y = "Hours"
  )
```
...
double_header <- cle_games_ts %>%
  ...
  index_by(yr = ~ year(.)) %>%
  group_by(yr, doubleheader_ind) %>%
  summarize(hours_per_9 = mean(hours_per_9, na.rm = TRUE), n = n())

double_header_effect <- double_header %>%
  as_tibble() %>%
  pivot_wider(id_cols = yr, names_from = doubleheader_ind, values_from = hours_per_9) %>%
...
cle_season_ts %>%
  ggplot(aes(x = yr)) +
  geom_line(aes(y = avg_hours_per_9), linewidth = 1, color = cle_colors["red"]) +
  geom_col(aes(y = avg_runs_per_9 / SF), width = 1.0, fill = cle_colors["navy_blue"], alpha = 0.2) +
  scale_x_continuous(breaks = seq(1900, 2030, 10)) +
  scale_y_continuous(limits = c(0, NA), n.breaks = 4,
...
cle_season_ts %>%
  ...
  ggplot(aes(x = yr)) +
  geom_line(aes(y = avg_hours_per_9), linewidth = 1, color = cle_colors["red"]) +
  geom_line(aes(y = PA / SF - 5), linewidth = 1, color = cle_colors["navy_blue"]) +
  scale_x_continuous(breaks = seq(1900, 2030, 10)) +
  scale_y_continuous(limits = c(0, NA), n.breaks = 4,
  ...
  labs(x = NULL, y = "Hours", color = NULL, fill = NULL,
   ...
   theme(
     axis.text.y.left = element_text(color = cle_colors["red"]),
     axis.text.y.right = element_text(color = cle_colors["navy_blue"], margin = margin(2, 2, 2, 2)),
     axis.title.y.left = element_text(color = cle_colors["red"]),
     axis.title.y.right = element_text(color = cle_colors["navy_blue"], margin = margin(l = 8)),
     plot.caption = element_text(family = "Calibri Light", size = 11),
   )
```
...
cle_season_ts %>%
  ...
  ggplot(aes(x = yr)) +
  geom_line(aes(y = avg_hours_per_9), linewidth = 1, color = cle_colors["red"]) +
  geom_line(aes(y = P / SF - 0), linewidth = 1, color = cle_colors["navy_blue"]) +
  scale_x_continuous(breaks = seq(1900, 2030, 10)) +
  scale_y_continuous(limits = c(0, NA), n.breaks = 4,
